...
function renderPlaylistThumb(item) {
  if (item.isWeb) return `<span class="playlist-thumb text-thumb">WEB</span>`;
...
function renderLivePreviewHero(device) {
  ...
  if (item.isWeb) {
    return `<div class="asset-preview video-thumb live-hero"><div class="video-icon">WEB</div><span class="video-label">${escapeHtml(item.name || "Web Page")}</span></div>`;
  }
  if (item.isYouTube) {
    ...
    <article class="playlist-card" data-library-item="${escapeAttr(item.id)}">
      ...
      <div class="playlist-strip">
        ${item.type === "youtube"
          ...
          : item.type === "web"
          ? `<span class="playlist-thumb text-thumb">WEB</span>`
          : item.type === "video"
